// ============================================================
// React 자료 08~15단원(실습프로젝트 .jsx) 검증기
// ------------------------------------------------------------
...
(async () => {
  ...
  for (const f of files) {
    let idx = await findIndex(f.unit.replace(/_/g, " "), f.label);
    ...
    // ★ 예제마다 페이지를 새로 연다.
    //   SPA 안에서 이동만 하면 앞 예제의 흔적이 남아 문제를 가린다.
    //   React 는 같은 이름의 컴포넌트에 대한 key 경고를 한 번만 낸다. 그래서
    //   05단원 개념03의 BadList 에서 한 번 나면 연습문제의 BadList 는 조용해지고,
    //   "경고허용으로 선언했는데 안 났다" 는 헛 지적이 나온다.
    // ★ 로그 통을 goto 前에 비운다.
    //   App 은 페이지가 열리자마자 '첫 예제' 를 저절로 고른다. 그래서 goto 뒤에
    //   비우면 그 첫 예제(01단원 개념01)가 모듈 실행 중에 찍은 줄이 통째로 사라진다.
    //   모듈은 한 번만 실행되므로 나중에 다시 골라도 그 줄은 안 나온다.
    logs = [];
    errors = [];
    warns = [];
    pending = [];
    ...
    idx = await findIndex(f.unit.replace(/_/g, " "), f.label);
    ...
    // 초시계처럼 1~2초 뒤에 결과를 찍는 예제가 있다. 넉넉히 기다린다.
