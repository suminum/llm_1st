...
function CounterWithState() {
  ...
  function handleAdd() {
    // 규칙이 여기에 한 벌
    if (count >= 10) return;//고치기1111111
    setCount(count + 1);
  }

  function handleRemove() {
    // 규칙이 여기에 또 한 벌
    if (count <= 0) return;//고치기222222222
    setCount(count - 1);
  }
...
function CounterWithReducer() {
  const [count, dispatch] = useReducer(counterReducer, 0);//0이 count를 의미함
...
function amountReducer(state, action) {//action 은 그냥 객체 받는 이르 ㅁ저기에 객체 담김 
  switch (action.type) {
    case "addMany":
